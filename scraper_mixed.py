"""Route scraping by chain: Marriott (StayAPI) + Arbitrip (HTZone)."""
import json
import traceback

from scraper_api import hotel_configs, scrape_hotel_api
from scraper_arbitrip import scrape_hotel_arbitrip
...
def scrape_hotel(hotel_config: dict, defaults: dict) -> dict:
    ...
    try:
        if chain == "arbitrip":
            return scrape_hotel_arbitrip(merged)
        ...
        return {
            ...
            "source": "arbitrip" if chain == "arbitrip" else "stayapi",
            "available": False,
            "rate": f"Error: {e}",
            "per_night": None,
            "currency": merged.get("currency", "ILS"),
            "error": str(e),
        }
